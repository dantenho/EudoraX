...
export const GeminiTxt2Img: React.FC = () => {
        ...
        if (token) {
          await fetch('/api/assets', {
            ...
            body: JSON.stringify({
              ...
              url: imageUrl,
              tags: ['generated', selectedStyle.id !== 'none' ? selectedStyle.id : 'base'],
            })
          });
        }
      }
        ...
        <div className="flex-1 bg-[#050507] rounded-3xl flex items-center justify-center relative overflow-hidden group border border-white/5 shadow-inner">
          {loading ? (
            ...
            <div className="text-zinc-800 flex flex-col items-center gap-6 opacity-[0.05]">
              <Icons.Palette className="w-32 h-32" />
              <p className="text-sm font-black uppercase tracking-[0.6em]">Synthesis Forge Ready</p>
            </div>
          )}
        ...
        <div className="flex gap-6 items-end">
          ...
          <div className="w-48 space-y-4">
             <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                <p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mb-2">Style Model</p>
                <select 
                   value={selectedStyle.id}
                   onChange={(e) => setSelectedStyle(LORA_STYLES.find(s => s.id === e.target.value) || LORA_STYLES[0])}
                   className="w-full bg-black border border-white/10 rounded-lg p-2 text-[10px] text-white focus:outline-none"
                >
                   {LORA_STYLES.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
             </div>
             <Button 
                loading={loading} 
                onClick={handleSynthesize} 
                className="h-16 w-full bg-blue-600 rounded-2xl shadow-[0_0_30px_rgba(37,99,235,0.2)] hover:shadow-[0_0_40px_rgba(37,99,235,0.4)]" 
                icon={Icons.Zap}
             >
                Forge
             </Button>
