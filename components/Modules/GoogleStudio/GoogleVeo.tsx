...
/**
 * @file GoogleVeo.tsx
 * @description Interface for Veo Temporal Motion Synthesis.
 */
...
export const GoogleVeo: React.FC<Props> = ({ loading, result, prompt, onPromptChange, onExecute }) => {
  return (
      ...
      <div className="flex-1 bg-black/40 border border-white/5 rounded-[2rem] overflow-hidden flex flex-col shadow-2xl">
        <div className="flex-1 flex items-center justify-center bg-[#030305] p-4 relative">
          {loading ? (
            <div className="text-center">
              ...
              <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Rendering Motion...</p>
            </div>
          ) : result ? (
            <video src={result} controls autoPlay loop className="max-h-full max-w-full rounded-xl" />
          ) : (
        ...
        <div className="p-8 bg-dark-sidebar/20 border-t border-white/5 flex gap-8 items-end">
          <div className="flex-1 space-y-4">
            <label className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">Motion Command</label>
            <textarea
              value={prompt}
              onChange={(e) => onPromptChange(e.target.value)}
              placeholder="Describe motion for Whisk Veo..."
              className="w-full h-24 bg-dark-bg/60 border border-white/5 rounded-2xl p-4 text-sm text-white resize-none"
            />
          </div>
          <Button onClick={onExecute} loading={loading} className="w-48 h-14 bg-blue-600" icon={Icons.Video}>Render</Button>
        </div>
      </div>
    </div>
  );
